...
class StreamPayKztService implements PaymentSystemInterface
{
    private $baseUrl;
    private $primaryKey;
    private $storeId;
    ...
    public function createOrder($orderId, $amount, $currency, $systemId)
    {
        try {
            ...
            // Конвертируем сумму в KZT
            $amountKZT = $amount;
            if ($currencySymbol != 'KZT') {
                $amountKZT = $this->exchangeService->convert($amount, $currencySymbol, 'KZT');
            }
            // Получаем информацию о KZT
            $KZTCurrency = collect($availableCurrencies)->firstWhere('code', 'KZT');
            if (!$KZTCurrency) {
                throw new Exception('KZT currency is not available');
            }

            // Проверяем лимиты для KZT
            if ($amountKZT < $KZTCurrency['min_amount'] || $amountKZT > $KZTCurrency['max_amount']) {
                throw new Exception("Amount must be between {$KZTCurrency['min_amount']} and {$KZTCurrency['max_amount']} KZT");
            }

            // Подготавливаем данные для инвойса
            $invoiceData = [
                'amount' => round($amountKZT, $KZTCurrency['precision']),
                'currency' => 'KZT',
                'store_id' => 405,
