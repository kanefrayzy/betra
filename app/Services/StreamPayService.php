...
class StreamPayService implements PaymentSystemInterface
{
    private $baseUrl;
    private $primaryKey;
    private $storeId;
    ...
    public function createOrder($orderId, $amount, $currency, $systemId)
    {
        try {
            ...
            // Конвертируем сумму в AZN
            $amountAzn = $amount;
            if ($currencySymbol != 'AZN') {
                $amountAzn = $this->exchangeService->convert($amount, $currencySymbol, 'AZN');
            }
            // Получаем информацию о AZN
            $aznCurrency = collect($availableCurrencies)->firstWhere('code', 'AZN');
            if (!$aznCurrency) {
                throw new Exception('AZN currency is not available');
            }

            // Проверяем лимиты для AZN
            if ($amountAzn < $aznCurrency['min_amount'] || $amountAzn > $aznCurrency['max_amount']) {
                throw new Exception("Amount must be between {$aznCurrency['min_amount']} and {$aznCurrency['max_amount']} AZN");
            }

            // Подготавливаем данные для инвойса
            $invoiceData = [
                // 'amount' => $amountAzn == 5
                // ? round($amountAzn + mt_rand(1, 5) / 100, $aznCurrency['precision'])
                // : round($amountAzn, $aznCurrency['precision']),
                'amount' => round($amountAzn, $aznCurrency['precision']),
                'currency' => 'AZN',
                'store_id' => 405,
