...
class NewGames extends BaseComponent
{
    public $isMobile;
    public $perPage = 30;
    ...
    private function getGames()
    {
        $query = empty($this->query)
            ? SlotegratorGame::where('is_new', true)
            : SlotegratorGame::search($this->query)->where('is_new', true);
    ...
    {
        return Cache::remember('new_providers', now()->addDay(), function () {
