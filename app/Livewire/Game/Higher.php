...
class Higher extends BaseComponent
{
    public $isMobile;
    public $perPage = 30;
    ...
    private function getGames()
    {
        $query = empty($this->query)
            ? SlotegratorGame::where('is_higher', true)->where('is_active', true)
            : SlotegratorGame::search($this->query)->where('is_higher', true)->where('is_active', true);
    ...
    {
        return Cache::remember('higher_providers', now()->addHours(20), function () {
            return SlotegratorGame::select('provider')
                ->selectRaw('count(*) as games_count')
                ->where('is_higher', true)
                ->groupBy('provider')
                ->get();
        });
    }
