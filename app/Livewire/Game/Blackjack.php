...
class Blackjack extends BaseComponent
{
    public $isMobile;
    public $perPage = 30;
    ...
    private function getGames()
    {
        $query = empty($this->query)
            ? SlotegratorGame::where('type', 'blackjack')->where('is_active', true)
            : SlotegratorGame::search($this->query)->where('type', 'blackjack')->where('is_active', true);
