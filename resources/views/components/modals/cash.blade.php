...
<div x-data="cashModalData()"
    ...
    <div x-show="open"
        ...
        <div class="relative px-6 py-4 border-b border-[#1a2c38]">
            <h2 class="text-2xl font-bold text-white">{{__('Кошелек')}}</h2>
            <button @click="closeModal()"
                    class="absolute top-4 right-6 text-gray-500 hover:text-white transition">
        ...
        <div class="relative grow px-6 min-w-0 max-w-full overflow-y-auto min-h-0 flex flex-col">
            
            <div class="flex w-full min-h-[48px] rounded-xl bg-[#1a2c38] p-0.5 my-4">
                <button @click="switchTab('deposit')"
                        type="button"
                        class="grow h-11 rounded-lg px-4 transition-all duration-300 text-sm font-semibold"
                        :class="operation === 'deposit' ? 'bg-[#3b82f6] text-white' : 'text-gray-400 hover:text-white'">
                    {{__('Депозит')}}
                </button>
                <button @click="switchTab('withdrawal')"
                        type="button"
                        class="grow h-11 rounded-lg px-4 transition-all duration-300 text-sm font-semibold"
                        :class="operation === 'withdrawal' ? 'bg-[#3b82f6] text-white' : 'text-gray-400 hover:text-white'">
                    {{__('Вывод')}}
                </button>
            </div>
            ...
            <div x-show="operation === 'deposit'" class="grow pb-4">
                ...
                <div x-show="showCryptoAddress">
                    ...
                    <div x-show="loadingCryptoAddress" class="flex flex-col items-center justify-center py-12">
                        <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-[#3b82f6] mb-4"></div>
                        <p class="text-gray-400">{{__('Загрузка адреса...')}}</p>
                    </div>

                    <div x-show="!loadingCryptoAddress && cryptoAddressData" class="bg-[#1a2c38] rounded-xl p-6 border border-[#2d3748]">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-lg font-bold text-white">{{__('Криптовалютный платеж')}}</h3>
                            <span class="px-3 py-1 bg-[#3b82f6]/20 text-[#3b82f6] text-sm font-semibold rounded-lg" 
                        ...
                        <div class="mb-4 p-3 bg-[#0f212e] rounded-lg border border-[#2d3748]">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <input type="text" 
                                           ...
                                           readonly
                                           class="flex-1 px-4 py-3 bg-[#0f212e] border border-[#2d3748] rounded-lg text-white text-sm focus:outline-none focus:border-[#3b82f6] font-mono break-all">
                                    <button @click="copyAddress()" type="button"
                                            class="px-4 py-3 bg-[#3b82f6] hover:bg-[#2563eb] text-white font-semibold rounded-lg transition-colors flex-shrink-0">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            ...
                            <div x-show="cryptoAddressData?.dest_tag">
                                ...
                                <div class="flex gap-2">
                                    <input type="text" 
                                           ...
                                           readonly
                                           class="flex-1 px-4 py-3 bg-[#0f212e] border border-[#2d3748] rounded-lg text-white text-sm focus:outline-none focus:border-[#3b82f6] font-mono">
                            ...
                            <div x-show="cryptoAddressData?.network">
                                ...
                                <div class="px-4 py-3 bg-[#0f212e] border border-[#2d3748] rounded-lg text-white text-sm font-semibold">
                        ...
                        <div class="mt-6 space-y-3">
                            ...
                            <div class="p-4 bg-[#3b82f6]/10 border border-[#3b82f6]/20 rounded-lg">
                                <div class="flex gap-3">
                                    <svg class="w-5 h-5 text-[#3b82f6] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div x-show="showBankPayment">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center w-full p-3 bg-[#1a2c38] border border-[#2d3748] rounded-xl">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <input type="number"
                                   class="w-full bg-[#1a2c38] border border-[#2d3748] focus:border-[#3b82f6] focus:ring-1 focus:ring-[#3b82f6] rounded-xl py-4 px-4 pr-16 text-white text-2xl font-bold placeholder-gray-500 focus:outline-none transition-all duration-200 text-center"
                                   x-model.number="amount"
                                   @focus="if(amount === 0) amount = ''"
                                   placeholder="0"
                                   required
                                   min="1">
                            <div class="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#3b82f6] font-bold text-xl">
                                {{$u->currency->symbol}}
                            </div>
                        </div>
                    ...
                    <div class="mb-6">
                        <div class="flex flex-wrap gap-2">
                            ...
                            <button type="button"
                                    ...
                                    :class="amount === {{ $quickAmount }} ? 'bg-[#3b82f6] text-white' : 'bg-[#1a2c38] text-white hover:bg-[#2d3748] border border-[#2d3748]'">
                    ...
                    <div class="flex items-center justify-between mb-4 p-4 bg-[#1a2c38] rounded-xl border border-[#2d3748]">
                    ...
                    <button @click="submitDeposit()"
                            type="button"
                            class="w-full h-14 bg-[#4dda30] hover:bg-[#3bb825] text-white font-bold rounded-xl transition-all duration-200 shadow-lg shadow-[#4dda30]/20 disabled:opacity-50 disabled:cursor-not-allowed text-lg"
                ...
                <div x-show="!showCryptoAddress && !showBankPayment">
                    ...
                    <p class="text-base font-semibold mb-3 mt-4 text-white">{{__('Банковский платеж')}}</p>
                    ...
                    <p class="text-base font-semibold mb-3 mt-4 text-white">{{__('Криптовалюта')}}</p>
                    <div class="flex flex-wrap gap-3">
            ...
            <div x-show="operation === 'withdrawal'" class="grow pb-4">
                ...
                <div x-show="showWithdrawalForm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                               class="w-full bg-[#1a2c38] border border-[#2d3748] focus:border-[#3b82f6] focus:ring-1 focus:ring-[#3b82f6] rounded-xl py-3 px-4 text-white placeholder-gray-500 focus:outline-none transition-all duration-200"
                               x-model="details"
                    ...
                    <div x-show="amount > 0" class="mb-4">
                        <div class="bg-[#1a2c38] rounded-xl p-4 border border-[#2d3748]">
                            <div class="space-y-2 text-sm">
                                ...
                                <div class="flex justify-between pt-2 border-t border-[#2d3748]">
                                    <span class="text-white font-semibold">{{__('К получению')}}:</span>
                                    <span class="text-[#4dda30] font-bold text-xl" x-text="actualAmount.toFixed(2) + ' {{$u->currency->symbol}}'"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div x-show="!showWithdrawalForm">
                    <p class="text-base font-semibold mb-3 mt-4 text-white">{{__('Способы вывода')}}</p>
                    <div class="flex flex-wrap gap-3">
