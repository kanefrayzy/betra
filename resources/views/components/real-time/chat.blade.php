<div id="right-sidebar"
    ...
    class="fixed inset-y-0 right-0 z-30 flex h-full w-full sm:w-96 md:w-80 lg:w-80 transform flex-col bg-dark-900 transition-all duration-500 ease-in-out"
     :class="{'translate-x-0 opacity-100': $store.ui?.chatOpen, 'translate-x-full opacity-0': !$store.ui?.chatOpen}"
     x-cloak
     @auth
        ...
        @auth
            <div class="relative mb-2 sm:mb-3">
                <input type="text"
                       ...
                       class="w-full rounded-xl bg-[#0f212e] border-2 border-[#2f4553] py-1.5 sm:py-2 pl-3 sm:pl-4 pr-10 sm:pr-12 text-sm sm:text-base text-white placeholder-gray-500 focus:outline-none focus:ring-0 focus:border-[#3d5563] transition-all duration-200"
                       maxlength="160">


                <button id="emoji-button" class="absolute right-3 sm:right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#ffb300] transition-colors duration-200 select-none active:scale-100" style="transform: translate(0, -50%);">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex items-center justify-between">
                <div id="char-counter" class="text-xs text-gray-400 px-1.5 sm:px-2 py-1" title="{{ __('Осталось символов') }}">160</div>
                
                <div class="flex items-center space-x-1.5 sm:space-x-2">
                    <a href="#" onclick="openChatRules();" class="flex items-center justify-center bg-[#2f4553] px-2.5 sm:px-3 py-1.5 rounded-lg text-gray-400 hover:text-white transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 sm:h-4 sm:w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <button id="send-message" class="rounded-lg bg-[#00e701] px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-medium text-black shadow-sm hover:shadow-[#ffb300]/25 transition-all hover:scale-105">
