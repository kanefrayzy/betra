		...
		columns: [
			...
			{ data: "username", searchable: false,
				render: function (data, type, row) {
					return '<img src="'+ row.avatar +'" style="width:50px;border-radius:50%;margin-right:10px;vertical-align:middle;">' + row.username;
				}
			...
			{ data: null, searchable: false, orderable: false,
				render: function (data, type, row) {
					...
					if(row.is_moder) return 'Moderator';
					if(row.is_youtuber) return 'YouTube`r';
					return 'Пользователь';
				}
			...
			{ data: null, searchable: false, orderable: false,
				render: function (data, type, row) {
					return '<a href="/' + cpBaseUrl +'/user/'+ row.id +'" class="btn btn-primary btn-rounded btn-xs">Изменить</a>';
				}
			}
		],
