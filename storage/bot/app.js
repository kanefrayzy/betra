var fs = require('fs'),
    options = {
        key: fs.readFileSync('/etc/letsencrypt/live/martinkazino.com/privkey.pem', 'utf8'),
        cert: fs.readFileSync('/etc/letsencrypt/live/martinkazino.com/fullchain.pem', 'utf8')
    },
	...
	animals          = new (require('./animals'))(io, config.domain),
	online 			= 1000,
	ipsConnected	= [];
...
redis.subscribe('real-time.clear');
...
redis.on('message', function(channel, message) {
    if (channel == 'real-time.clear') {
        log('[CHAT] Чат очищен!');
        io.sockets.emit('clear', message);
    }
    if (channel == 'new.msg') {
        io.sockets.emit('real-time', message);
    }
	if (channel == 'del.msg') {
	...
	if(channel == 'animals' && JSON.parse(message).type == 'back_timer') {
        message = JSON.parse(message);
        return animals.startTimer(message.timer);
    }
   /* if (channel == 'new.flip') {
...
function fakeBetJackpot(status) {
	if(status) {
		...
		.then(function(res) {
			...
			}, Math.round(getRandomArbitrary(2, 8) * 1000));
		}, function(res) {
...
function fakeBetAnimals(status) {
	if(status) {
		requestify.post(config.domain + '/api/animals/addBetFake')
		.then(function(res) {
			res = JSON.parse(res.body);
			if(res.time <= 3) log(res.msg);
			if(res.success) log(res.msg);
			else log(res.msg);
			setTimeout(function() {
				fakeBetAnimals(res.fake);
			}, Math.round(getRandomArbitrary(1, 2) * 400));
		}, function(res) {
			log('[Animals] Ошибка при добавлении ставки!');
			log(JSON.parse(res.body));
			setTimeout(function() {
				fakeBetAnimals(1);
			}, Math.round(getRandomArbitrary(1, 2) * 400));
